...
import { handleGetMagnetPerformance, handleListMagnets } from "./api/magnets.js";
...
const server = createServer(async (req, res) => {
  ...
  const magnetPerformanceMatch = /^\/api\/magnets\/([^/]+)\/performance$/.exec(url.pathname);
  if (req.method === "GET" && magnetPerformanceMatch) {
    await handleGetMagnetPerformance(req, res, magnetPerformanceMatch[1] ?? "");
    return;
  }
