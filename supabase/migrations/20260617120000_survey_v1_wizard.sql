-- Survey Campaign v1 wizard 重构：状态机扩展 + 题型扩展 + 字段拆分
-- 依据 survey define.md（不含奖励系统）

-- =====================================================================
-- 1. q_survey_campaigns：新增字段 + 扩展 status 状态机
-- =====================================================================

ALTER TABLE q_survey_campaigns
  ADD COLUMN IF NOT EXISTS intro_text     TEXT,
  ADD COLUMN IF NOT EXISTS frequency_cap  TEXT NOT NULL DEFAULT 'once_per_user',
  ADD COLUMN IF NOT EXISTS timezone       TEXT;

-- frequency_cap 取值约束
ALTER TABLE q_survey_campaigns
  DROP CONSTRAINT IF EXISTS q_survey_campaigns_frequency_cap_check;
ALTER TABLE q_survey_campaigns
  ADD CONSTRAINT q_survey_campaigns_frequency_cap_check
  CHECK (frequency_cap IN ('once_per_user', 'once_per_day', 'once_per_round'));

-- 扩展 status CHECK：保留 draft/review/active/paused/archived，新增 ready_to_publish/scheduled/ended
DO $$
DECLARE c text;
BEGIN
...
ALTER TABLE q_survey_campaigns
  ...
  CHECK (status IN (
    'draft', 'review', 'ready_to_publish', 'scheduled',
    'active', 'paused', 'ended', 'archived'
  ));

-- =====================================================================
-- 2. q_survey_questions：扩展题型 + rating_scale，迁移历史 multi_choice
-- =====================================================================

ALTER TABLE q_survey_questions
  ADD COLUMN IF NOT EXISTS rating_scale INT;

-- 先删旧 question_type CHECK，再迁移数据，再加新 CHECK
DO $$
DECLARE c text;
BEGIN
  SELECT conname INTO c
...
UPDATE q_survey_questions
  SET question_type = 'multiple_choice'
  WHERE question_type = 'multi_choice';

ALTER TABLE q_survey_questions
  ADD CONSTRAINT q_survey_questions_question_type_check
  CHECK (question_type IN (
    'single_choice', 'multiple_choice', 'rating', 'yes_no', 'short_text'
  ));

-- rating_scale 合法范围（仅当设置时）
ALTER TABLE q_survey_questions
  DROP CONSTRAINT IF EXISTS q_survey_questions_rating_scale_check;
ALTER TABLE q_survey_questions
  ADD CONSTRAINT q_survey_questions_rating_scale_check
  CHECK (rating_scale IS NULL OR (rating_scale >= 2 AND rating_scale <= 10));

-- =====================================================================
-- 3. q_survey_answer_events：放宽“必须有 option”约束
--    rating 用 selected_value、short_text 用 other_text，均无 option_id
-- =====================================================================
...
BEGIN
  SELECT conname INTO c
  FROM pg_constraint
  WHERE conrelid = 'q_survey_answer_events'::regclass
    AND contype = 'c'
    AND pg_get_constraintdef(oid) ILIKE '%survey_option_id%';
  IF c IS NOT NULL THEN
    EXECUTE format('ALTER TABLE q_survey_answer_events DROP CONSTRAINT %I', c);
  END IF;
END $$;
ALTER TABLE q_survey_answer_events
  ADD CONSTRAINT q_survey_answer_events_answer_present_check
  CHECK (
    action = 'skipped'
    OR survey_option_id IS NOT NULL
    OR selected_value IS NOT NULL
    OR other_text IS NOT NULL
  );

-- =====================================================================
-- 4. RPC：question 列表 JSON 增加 ratingScale / type，campaign 增加 introText
-- =====================================================================
