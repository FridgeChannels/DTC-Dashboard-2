-- Batch issuance is atomic: any failure rolls back all assignments in the request.

CREATE OR REPLACE FUNCTION public.fc_raise_realtime_coupon_error(
  p_message TEXT,
  p_status_code INT
)
RETURNS VOID
...
BEGIN
  ...
  -- Phase 1: validate every campaign before mutating coupon state.
  FOREACH v_campaign_id IN ARRAY v_campaign_ids LOOP
    ...
    IF NOT v_is_available THEN
      ...
      IF FOUND
        ...
        PERFORM public.fc_raise_realtime_coupon_error(
    ...
    IF NOT FOUND THEN
      PERFORM public.fc_raise_realtime_coupon_error(
        'campaign_id ' || v_campaign_id || ' not found',
        404
      );
    END IF;

    IF v_campaign.status <> 'active' THEN
      PERFORM public.fc_raise_realtime_coupon_error('Campaign is not active', 400);
    END IF;
    ...
    IF v_campaign.distribution_mode = 'shared_code' THEN
      IF NOT EXISTS (
        ...
        PERFORM public.fc_raise_realtime_coupon_error(
          'No shared coupon code configured for this campaign',
          404
        );
      END IF;
    ELSIF NOT EXISTS (
      ...
      PERFORM public.fc_raise_realtime_coupon_error(
        'No available coupon codes for this campaign',
        404
      );
    END IF;
    ...
    IF COALESCE(
      (
        ...
        WHERE code.customer_id = v_customer_id
          AND code.campaign_id = v_campaign.campaign_id
          AND (
            (v_campaign.distribution_mode = 'shared_code'
              AND code.usage_mode = 'shared'
              AND code.status IN ('available', 'assigned'))
            OR (v_campaign.distribution_mode <> 'shared_code' AND code.status = 'available')
          )
        ORDER BY code.created_at ASC
        LIMIT 1
      ),
      v_campaign.shopify_discount_node_id
    ) IS NULL THEN
      PERFORM public.fc_raise_realtime_coupon_error(
        'Campaign is not linked to Shopify discount',
        500
      );
    END IF;
  END LOOP;

  -- Phase 2: assign all campaigns in one transaction; any failure rolls back everything.
  FOREACH v_campaign_id IN ARRAY v_campaign_ids LOOP
    SELECT *
    INTO v_campaign
    FROM public.fc_coupon_campaign c
